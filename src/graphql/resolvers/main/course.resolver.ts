...
export const courseResolver: BaseContext = {
		...
		getCourses: async function (
			...
			try {
				...
				for (const course of courses) {
					...
					const courseRating = await CourseRatingService.getOne({
						course_id: course.id,
					}) as ICourseRating;

					course.rating = courseRating.rating || 0;
	...
	Mutation: {
		getCourse: async function (
			_: any,
			{ getCourseInput }: { getCourseInput: IGetCourse },
			context: any
		): Promise<IGetCourseResponse> {
			try {
				const { page, limit }: IPagination = context.req.query;
				const course: ICourse = (
					await CourseService.getSearchCourses(getCourseInput, {
						page,
						limit,
					})
				)[0];
				...
				const foundThemes: ICourseThemeWithVideos[] =
					await CourseThemeService.getCourseThemes({
						course_id: course.id,
					});
